...
LOGS_FOLDER="/vars/logs/shellscript-logs"
...
if [ $USERID -ne 0 ]
 then
    echo -e "$R ERROR:: Please run this script with root user $N" &>>$LOG_FILE
    exit 1
else
...
then
    ...
    echo -e "Nothing to do MYSQL is... $Y already installed... $N" &>>$LOG_FILE
fi
...
then
    ...
    echo -e "Nothing to do Python3 is... $Y already installed... $N" &>>$LOG_FILE
fi
...
then
    ...
    echo -e "Nothing to do nginx is... $Y already installed... $N" &>>$LOG_FILE
fi